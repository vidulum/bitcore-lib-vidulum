...
describe('Unit', function() {
  ...
  it('has property accesors "VDL", "mVDL", "uVDL", "bits", and "satoshis"', function() {
    var unit = new Unit(1.2, 'VDL');
    unit.VDL.should.equal(1.2);
    unit.mVDL.should.equal(1200);
    unit.uVDL.should.equal(1200000);
  ...
  it('a string amount is allowed', function() {
    ...
    unit = Unit.fromVDL('1.00001');
    unit.VDL.should.equal(1.00001);

    unit = Unit.fromMilis('1.00001');
    unit.mVDL.should.equal(1.00001);
    ...
    unit.VDL.should.equal(0.12285714);
  });
  ...
  it('exposes unit codes', function() {
    should.exist(Unit.VDL);
    Unit.VDL.should.equal('VDL');

    should.exist(Unit.mVDL);
    Unit.mVDL.should.equal('mVDL');
  ...
  it('exposes a method that converts to different units', function() {
    var unit = new Unit(1.3, 'VDL');
    unit.to(Unit.VDL).should.equal(unit.VDL);
    unit.to(Unit.mVDL).should.equal(unit.mVDL);
  ...
  it('exposes shorthand conversion methods', function() {
    var unit = new Unit(1.3, 'VDL');
    unit.toVDL().should.equal(unit.VDL);
    unit.toMilis().should.equal(unit.mVDL);
    unit.toMillis().should.equal(unit.mVDL);
  ...
  it('can be imported and exported from/to JSON', function() {
    var json = JSON.stringify({amount:1.3, code:'VDL'});
  ...
  it('fails when the unit is not recognized', function() {
    ...
    expect(function() {
      return new Unit(100, 'VDL').to('USD');
    }).to.throw(errors.Unit.UnknownCode);
  });
  ...
  it('fails when the exchange rate is invalid', function() {
    ...
    expect(function() {
      return new Unit(100, 'VDL').atRate(-123);
    }).to.throw(errors.Unit.InvalidRate);
  });
